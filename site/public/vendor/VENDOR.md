...
| `hechima-wasm-st/`（hechima-wasm.js / .wasm。**検証用・辞書は上の `hechima-wasm/mozc.data` を共有**） | **単スレッド版**（-pthread なし） | labo `verify/wasm-single-thread` `c0e13b9` の CI artifact `hechima-wasm-singlethread`（run 30134214247）。provenance: fcitx5-mozc `522a5f2` / emsdk 3.1.69 / `patches/single_thread.patch` 適用（同梱 BUILD_INFO.txt）。**SharedArrayBuffer / pthread の参照ゼロ = COOP/COEP 不要**。`/coi-test/` からのみ参照（本番ページは pthread 版のまま）。init 737ms → 156ms・js -19%・変換速度は同等（labo `hechima-wasm/README.md` の「単スレッドビルド」節） |
