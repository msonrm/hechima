...
| ディレクトリ | 版 | 取得元 |
|---|---|---|
| `hechima/`（hechima.js / hechima-worker.js / hechima.d.ts） | **v0.12.0** | labo Release `hechima-v0.12.0`（合成中の confirm/insertAndConfirm = 無変換で即確定。薙刀式 V+M = Enter） |
...
- hechima v0.12.0 は **KeymapEngine >= 1.4.0 必須**（セット差し替え）
