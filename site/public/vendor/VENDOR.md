...
| ディレクトリ | 版 | 取得元 |
|---|---|---|
| `hechima/`（hechima.js / hechima-worker.js / hechima.d.ts） | **v0.13.1** | labo **PR #704**（`feat/chrome-ext-gamepad-vertical`）の `web/public/hechima/`。マージ後に Release `hechima-v0.13.1` を切る予定（切ったらこの欄を差し替えること）。v0.13.0 = `insertKana` = かな直接注入・フリック入力の配線先。**v0.13.1 = BS で素通しローマ字まで戻ったときの pending 復帰** — `knji` → 「kんじ」の `k` まで BS で戻して `a` を打つと「kあ」になっていたのを実 IME と同じ「か」に。公開 API 変更なし = 差し替えは hechima 単体で完結 |
