...
let qrFill = 0;
let decMs = 0;
/* ★★**自動ズームは入れて、外した**（2026-09-15）—— iPad が枠の 40% でしか写せず
 *   （v30 の 1 模様 2.0px）遅い原因だと見て寄らせたが、**暴走した**:
 *   ★寄りすぎて読めなくなると `qrFill` が更新されないので、**古い小さい値のまま
 *   「まだ小さい」と判断してさらに寄る**。止まらない。
 *   ★★そしてそもそも不要だった —— 遅さの正体は**既読の枚を何度もデコードしていたこと**で、
 *   40% でも読めてはいた。**読めるかどうかではなく、無駄なデコードが問題だった。**
 *   ★小ささの案内（「もっと近づけてください」）だけ残す ―― 人が寄る方が確実で、暴走しない。 */
 ...
 *   30fps で 24 フレームは **0.8 秒**しかなく、機体の 1 周（当時 17 枚 × 0.1 ＝ 1.7 秒）の
 *   半分しか見ていなかった。**「最後の数枚が埋まらない」の正体**がこれ。
 *   ★**1 巡で全枚が視界に入る**だけ溜める（64 フレーム ＝ 2.1 秒 ＝ 33MB）。
 * ★機体が 0.05 秒になって 1 周は 0.85 秒（17 枚）になったので、**撮り止めを決めるのは
 *   時間の側** ＝ この 64 は**安全弁**（指紋で既読を弾くので普段は届かない）。 */
const BANK_MAX = 64;
let need = 0;                     /* 総枚数（0 = まだ分からない ＝ 従来どおり 1 枚ずつ読む） */
/* ★★**機体が 1 枚を出しているあいだの時間**（ミリ秒）。★**1 周ではなく 1 枚あたり**
 *   ―― 1 周は `sheetMs × 総枚数`。
 * ★★**既定値は機体と揃えてある**（`QR_AUTO_FIELDS` = 3 ＝ 3 フィールド ＝ 0.05 秒。
 *   2026-09-16 に確定）—— **決まった値は URL に載せる必要がない**ので、機体は
 *   `&ms=` を渡してこなくなった（`?n=` だけ）。
 * ★`?ms=` が付いていればそちらに従う（機体側で `EDITOR_QR_FIELDS` を変えて
 *   試すとき、URL を手で足せば撮り止めも付いてくる）。 */
const SHEET_MS = 50;
let sheetMs = SHEET_MS;
...
/** まだ来ていない枚の数。★1 枚も読めていないうちは、機体が URL で教えてきた総枚数。 */
function rest(): number {
  return col.empty ? need : col.missing.length;
}

/* ★**1 枚あたり何フレーム撮れているか**を出す —— 機体の間隔はこの数から決めた。
   1 枚あたりの時間（既定 0.05 秒 / `?ms=` があればそちら）で割り算するだけ。 */
function showMeter(): void {
  if (!fps) {
    meterEl.textContent = "";
    return;
  }
  /* ★★**単位を混ぜない**（2026-09-16）—— この画面で「枚」は QR の枚だけを指す。
     カメラが撮るのは「フレーム」。★**同じ助数詞で違うものを数えると、並べた数どうしが
     食い違って見える。** */
  const perSheet = (fps * sheetMs) / 1000;
  let t = `カメラ ${fps} フレーム/秒 ／ QR 1 枚あたり ${perSheet.toFixed(1)} フレーム`;
  ...
  /* ★★★**「暗いかもしれません」は外した**（2026-09-16）—— 閾値の 2 は**機体が
     0.1 秒だった頃の数**（当時 30fps で 3.0）。0.05 秒に確定したいま、30fps のカメラは
     **必ず 1.5** になるので、**常に出る警告**になっていた。
     ★**常に出る警告は、何も言っていないのと同じ。**
     ★★**測った値で出す案内は、測る対象が動いたら一緒に見直す。**
     ★遅さそのものは下の fps で言えば足りる。 */
  if (fps < 20) t += "　★カメラが遅めです";
...
function drainStep(): void {
  ...
  if (!bank.length) {
    ...
    if (!done) say(col.empty ? "QR を枠に収めてください。" : `あと ${rest()} 枚。`);
  } else {
    /* ★★**フレーム数は出さない**（2026-09-16）—— ここは溜めた**フレーム**の本数を
       「残り 28」と出していたが、この画面のほかの数は全部「枚」なので、**単位の違う数が
       並んで**「あと 14 枚」と食い違って見えた。★しかも**読んでいるあいだも撮る**ので、
       1 本減って 1 本増える ＝ **数字が止まって見える**。
       ★撮っている人が知りたいのは「あと何枚か」だけ。 */
    say(`読み取り中…（あと ${rest()} 枚）`);
  }
}
...
function grab(): boolean {
  ...
  /* ★★★**同じ枚を何枚も溜めない**（2026-09-15・iPad で 30fps 出ても速くならなかった）——
     ...
     *違う枚* はせいぜい 7 枚。**15 フレームは同じ枚の撮り直し**で、それを全部デコード
     ...
     ★★★**「残り 2 枚以下なら間引きをやめる」は、指紋を入れた時点で不要になり、
     しかも害になっていた**（2026-09-15・実機「最後の 2 枚になった途端に別処理が
     動いている？」）—— その分岐は**溜められる上限も 64 → 12 に急変させていた**ので、
     ★**溜まっていた未読フレームが一気に捨てられ、次の周まで待たされていた**。
     「最後の 2 枚に差し掛かるところで必ずちょっと待ち、突破すれば速い」の正体。
     ★★**間引きは指紋がやっている** —— 未読の枚しか溜まらないのだから、時間で間引く
     必要も、上限を切り替える必要もない。**条件分岐そのものを消した。**
     ★分岐を消すと「その条件でだけ起きる不具合」も消える。

     ★周期の噛み合わせ（ビート）も疑ったが、模擬では否定された —— カメラの fps は
     正確な整数比にならないので位相は少しずつずれる。ランダムに揺らす案も効かない
     （カメラの刻みに量子化される）。**効かないと分かったものは入れない。** */
  ...
  while (bank.length >= BANK_MAX) bank.shift();   /* ★安全弁（指紋があるので普段は届かない） */
  ...
  /* ★★★**やめどきは「機体が 1 周するまで」**（2026-09-15 に直した）——
     ...
     見ていなかった。**1 周 ＋ 2 割**撮る。
     ★1 周は「1 枚あたりの時間 × 総枚数」で、1 枚あたりの時間は**機体の既定値**（0.05 秒）。
     ★以前は機体が `&ms=` で教えてきていたが、値が確定したので**こちらが知っている**。 */
  const want = Math.max(1, rest());
  const oneRound = sheetMs * need * 1.2;
   ...
   総枚数が最初から手元にある（ブックマークから開いた人には無いが、そのときは従来どおり）。 */
{
  ...
  /* ★機体はもう `&ms=` を渡してこない（0.05 秒で確定したため）。付いていれば従う
     ―― 間隔を変えて試すときは、URL に手で足せば撮り止めも付いてくる。 */
  const ms = Number(new URLSearchParams(location.search).get("ms"));
  if (Number.isInteger(ms) && ms > 0 && ms <= 2000) sheetMs = ms;
  if (n) {
    need = n;
